...
if ($this->_request->getMethod() == 'POST' && !isset($message)) {
	$this->addJs('
	parent.$("#contact-infos").text("' . $contact['nom'] . ' ' . $contact['prenom'] . ' (' . $contact['email'] . ')");
	parent.$("#contact").val(' . $contact['id'] . ');
	parent.$.colorbox.close();');
}
?>
			<p style="text-align: left;">
				<a href="<?=$this->c(Request::createUri('comptage:extraction:addcontact'))?>" class="add" title="ajouter un nouveau contact">Ajouter un contact</a>
			</p>
